...
```{r unique_ASVs}
# making a list of all ASVs in each sample
...
for (i in 1:nrow(tab)) {
    ...
    col_names <- colnames(tab)[col_indices] # find the ASV names 
    ASVs_sample[[i]] <- col_names # save the info in a list
}

# Find how common each ASV is
...
# Finding ASVs unique for each sample
unique_entries <- vector("list", length(ASVs_sample)) 
for (i in seq_along(ASVs_sample)) {
    current_vec <- ASVs_sample[[i]]
    unique_in_vec <- current_vec[entry_freq[current_vec] == 1] # finding which of the ASVs for a sample only appears once in the data
...
## Number of ASVs vs nr of reads
```{r}
nASV$reads <- rowSums(tab) #total amount of reads for a sample
nASV$Season <- metaData$Season
max(nASV$reads)
min(nASV$reads)

# finding if nr of reads correlates with nr of ASVs for samples
...
# Dot plot to visualize nr of reads and nr of ASVs
...
ps_class <- tax_glom(ps, "Class") # Collapse abundance to class level
class_abundance <- psmelt(ps_class) # make data more suited for plotting

# Group abundance by location and sampling season
ps_summary <- class_abundance %>%
  group_by(loc_season, Class) %>%
  summarise(Abundance = sum(Abundance)) %>%
  ungroup()

# Normalize the abundance data within each loc_season 
...
Class_abundance_plot <- ggplot(ps_summary %>% group_by(loc_season) %>% arrange(-Abundance, by_group = TRUE), aes(fill = Class, x = Abundance, y = loc_season, group = loc_season)) +  # Switch x and y
  ...
  scale_fill_manual(values = frg, guide = guide_legend(ncol = 7, title.position = "top")) +
  theme(plot.title = element_text(size = 40, face = "bold"),
        axis.title.x = element_text(size = 30),             
        axis.title.y = element_text(size = 30),             
        axis.text.x = element_text(size = 30),
        axis.text.y = element_text(size = 30, angle = 0, hjust = 0.5, vjust = 0.5),  # Rotate y-axis text
...
png("results/Class_abundance_plot.png", width = 2400, height = 1700)
print(Class_abundance_plot)
dev.off()
```

## Find abundance of bacterial classes in dataset
```{r}
most_abundant_classes <- ps_summary %>%
  group_by(Class) %>%
...
stress_s <- mod_s$stress # finding how well the data fits

# prepairing data to plot
...
metaData_fall_subset <- filter(metaData_fall, place %in% c("B", "C")) # select locations here
...
NMDS_plot <- ggplot(NMDS_data_f, aes(x = Dim1, y = Dim2, color = Location)) +
  ...
  geom_encircle(aes(group = Location, fill = Location), 
                ...
                alpha = 0.2, # Transparency of the fill
                s_shape = 1, 
                expand = 0) + # Control the tightness of the encircling lines
  scale_fill_manual(values = frg) +
  labs(x = 'Dimension 1', y = 'Dimension 2', shape = 'Sample Type') +  # Axis labels
  theme(text = element_text(size = 30),  # Global text size
    axis.title = element_text(size = 30),  # Axis titles
    axis.text = element_text(size = 30),  # Axis text
    legend.title = element_text(size = 30),  # Legend title
    legend.text = element_text(size = 30)   # Legend text
...
# Linking Samplegroup nr back to location 
group_list <- c(1:13)
...
#remove coluns not in use
permanova_tbl <- permanova_tbl %>%
  mutate(place1 = permanova_tbl$place_list.x, place2 = permanova_tbl$place_list.y)%>%
  subset(select= -c(place_list.x,place_list.y, group1, group2))

# Cleaning results table
pairwise_fall_permanova_tbl <- na.omit(permanova_tbl) #removing rows not showing explanatory variables
pairwise_fall_permanova_tbl$type <- gsub("[0-9]+", "", pairwise_fall_permanova_tbl$type)  # Removing nr from type column
...
# Check which locations do not show significant differences caused by location
permanova_sign_fall_loc <- filter(permanova_tbl1_0.5, type == "group")
table(permanova_sign_fall_loc$place1)
...
for (i in 1:(length(unique_groups))) {
  for (j in (i):length(unique_groups)) {
    ...
    # Remove distances between the same sample
    between_group_distances[between_group_distances == 0] <- NA # Assumes that only self comparisons are exactly 0
    mean_bc <- mean(between_group_distances, na.rm = TRUE)
...
# Finding if Bray-curtis distances within locations are different from between locations
...
# testing the if there is a difference
 wilcox.test(bc_different$value, bc_same$value, alternative = "two.sided")   
```
...
# Comparing samples from different timepoints within Locations
...
# testing if there is a difference between samples collected at the same time vs different times
...
```{r}
# Finding nr reads and ASVs for summer samples (not scaled)
...
# Finding nr of reads and ASVs for fall samples (not scaled)
...
# Rarefying summer data to mean read counts for fall data
fall_mean_reads <- mean(fall_info$total_reads)
rarefied_data <- as.data.frame(rrarefy(summer_data, fall_mean_reads))
print(rowSums(rarefied_data))


rarefied_data$nrASVs <- rowSums(rarefied_data > 0)
print(rarefied_data$nrASVs)
print(summer_info$nrASVs)

# Testing if there is a difference in nr of ASVs between fall data and rarefied summer data
...
# Separate data by sampling dag and season
...
# Differences in Bray-Curtis distances between samples collected within 2 weeks and across seasons
wilcox.test(sampling_day$MeanBrayCurtis, Season$MeanBrayCurtis, alternative = "two.sided")

# Differences in Bray-Curtis distances between samples collected at the same time and across seasons
spatial <- rbind(fall_spatial, summer_spatial)
...
# Perform Wilcoxon rank-sum test
## Differences in nr of ASVs between soil and fabric samples
nrASVs_result <- wilcox.test(fabric_info$nrASVs, soil_info$nrASVs, alternative = "greater")
print(nrASVs_result)

## Difference in nr of reads between soil and fabric samples
...
tax_Symbio <- subset(tax_summer, Class == "Symbiobacteriia")
symbio_counts <- tab_rar_summer[, colnames(tab_rar_summer) %in% rownames(tax_Symbio)]
print(symbio_counts)
```
